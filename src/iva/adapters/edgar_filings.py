...
async def lookup_cik(company_name: Optional[str] = None, ticker: Optional[str] = None) -> Optional[str]:
    """
    ...
    try:
        ...
        if company_name:
            ...
            name_clean = re.sub(r'\s+(inc\.|incorporated|corp\.|corporation|ltd\.|limited|llc|plc)\.?$', '', name_lower, flags=re.IGNORECASE)
...
def extract_latest_value(facts: Dict[str, Any], concept: str, form_type: str = "10-K") -> Optional[Dict[str, Any]]:
    """
        ...
        for idx, form in enumerate(forms):
            if form == form_type:
                return {
                    ...
                    "primary_doc_description": recent_filings.get("primaryDocDescription", [])[idx] if idx < len(recent_filings.get("primaryDocDescription", [])) else "",
                }
    ...
    try:
        ...
        if not cik:
            ...
            findings.append(AdapterFinding(
                ...
                observed_at=now,
                snippet=f"Could not find CIK for company '{company}'" + (f" or ticker '{ticker}'" if ticker else "") + ". Company may be private or use a different name.",
                citations=[Citation(
        ...
        # Step 3: Get latest 10-K (annual report)
        ...
        # Step 4: Get latest 10-Q (quarterly report)
        ...
        # Step 5: Get financial facts (XBRL data)
