...
def test_marketing_discrepancy_deduplicates_claims():
    cs = ClaimSet(url="u", company="c", extracted_at=datetime.now(UTC), claims=[
        ExtractedClaim(id="m1", category="marketing", claim_text="Millions of customers use us", claim_kind="customer_count"),
        ExtractedClaim(id="m2", category="marketing", claim_text="We process hundreds of billions", claim_kind="transaction_volume"),
    ])
    edgar_finding = AdapterFinding(
        ...
        citations=[Citation(source="EDGAR", url="https://sec.gov", query="q", accessed_at=datetime.now(UTC))]
    )
    news_finding = AdapterFinding(
        ...
        citations=[Citation(source="News", url="https://news.google.com", query="q", accessed_at=datetime.now(UTC))]
    )
    adapters = {
        "edgar": [edgar_finding],
        "news": [news_finding],
